...
export async function registerHospital(input: RegisterHospitalInput) {
  ...
  if (existing)
    throw Conflict("A hospital with this NIN is already registered");
  ...
  if (input.inviteToken) {
    try {
      const invInfo = await markInvitationSubmitted(
        input.inviteToken,
        hospital.id,
      );
      if (invInfo) {
      ...
      logger.error(
        { err, hospitalId: hospital.id },
        "failed to auto-approve invited hospital",
      );
    }
  }
...
async function autoApproveInvitedHospital(
  h: HospitalDoc,
  actorUserId: string,
): Promise<void> {
  h.status = "approved";
  ...
  try {
    ...
    logger.error(
      { err, hospitalId: h.id },
      "failed to mark invitation approved",
    );
  }
}
...
export async function getHospitalDocumentUrl(
  hospitalId: string,
  slotKey: string,
): Promise<string> {
  const h = await Hospital.findById(hospitalId).exec();
