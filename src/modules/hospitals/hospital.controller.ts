...
export async function patchSuspend(
  req: Request,
  res: Response,
  next: NextFunction,
) {
...
export async function patchReactivate(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    if (!req.user) throw Forbidden();
    const hospital = await service.reactivateHospital(
      req.params.id!,
      req.user.userId,
    );
    res.json(hospital);
...
export async function patchMyHospital(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    const hospital = await service.updateMyHospital(
      req.user.hospitalId,
      input,
      req.user.userId,
    );
...
export async function postMyPhoto(
  ...
  try {
    ...
    const hospital = await service.addHospitalPhoto(
      req.user.hospitalId,
      input,
      req.user.userId,
    );
...
export async function deleteMyPhoto(
  ...
  try {
    ...
    const hospital = await service.deleteHospitalPhoto(
      req.user.hospitalId,
      s3Key,
      req.user.userId,
    );
