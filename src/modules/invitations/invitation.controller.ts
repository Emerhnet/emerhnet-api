...
import { createInvitationSchema, listInvitationsSchema } from './invitation.schemas';
...
export async function postReissue(req: Request, res: Response, next: NextFunction) {
  try {
    if (!req.user) throw Forbidden();
    const result = await service.reissueInvitation(req.params.id!, req.user.userId);
    res.json(result);
  } catch (err) {
    next(err);
  }
...
export async function postCancel(req: Request, res: Response, next: NextFunction) {
  try {
    if (!req.user) throw Forbidden();
    const invitation = await service.cancelInvitation(req.params.id!, req.user.userId);
    res.json(invitation);
  } catch (err) {
    next(err);
  }
...
export async function getVerify(req: Request, res: Response, next: NextFunction) {
  try {
