...
export async function reissueInvitation(id: string, actorUserId: string) {
  ...
  if (inv.status === "submitted" || inv.status === "approved") {
    throw ValidationError(
      "Cannot reissue an invitation that has already been used",
    );
  }
...
export async function verifyInvitation(token: string) {
  ...
  if (inv.status === "cancelled")
    throw ValidationError("This invitation has been cancelled");
 ...
 */
export async function markInvitationApproved(
  hospitalId: string,
): Promise<void> {
