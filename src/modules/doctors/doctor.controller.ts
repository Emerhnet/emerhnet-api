...
export async function patchOne(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    res.json(await service.updateDoctor(hospitalId, req.params.id!, input, userId));
  } catch (err) {
    next(err);
  }
}
...
export async function postDeactivate(req: Request, res: Response, next: NextFunction) {
  try {
    const { hospitalId, userId } = ctx(req);
    res.json(await service.deactivateDoctor(hospitalId, req.params.id!, userId));
  } catch (err) {
    next(err);
  }
}
...
export async function postReactivate(req: Request, res: Response, next: NextFunction) {
  try {
    const { hospitalId, userId } = ctx(req);
    res.json(await service.reactivateDoctor(hospitalId, req.params.id!, userId));
  } catch (err) {
    next(err);
  }
}
