...
export async function postCreate(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    res.status(201).json(await service.createDepartment(hospitalId, input, userId));
  } catch (err) {
    next(err);
  }
}
...
export async function patchOne(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    res.json(await service.updateDepartment(hospitalId, req.params.id!, input, userId));
  } catch (err) {
    next(err);
  }
}
