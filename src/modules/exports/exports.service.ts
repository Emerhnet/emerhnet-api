...
export async function exportHospitalsCsv(): Promise<string> {
  ...
  const rows = docs.map((h) => [
    ...
    h.category,
    h.status,
    h.cghsEmpanelment ? "Yes" : "No",
    h.ayushmanEmpanelment ? "Yes" : "No",
    h.address.city,
    h.address.state,
    h.address.pincode,
    h.adminContact.name,
    h.adminContact.email,
    h.adminContact.phone,
    h.createdAt,
    h.approvedAt,
  ]);
...
export async function exportDoctorsCsv(hospitalId: string): Promise<string> {
  ...
  const rows = docs.map((d) => [
    ...
    d.gender,
    d.joinedAt,
    d.deactivatedAt ? "Deactivated" : "Active",
  ]);
  return rowsToCsv(headers, rows);
}
...
export async function exportAmbulancesCsv(hospitalId: string): Promise<string> {
  const docs = await Ambulance.find({ hospitalId })
